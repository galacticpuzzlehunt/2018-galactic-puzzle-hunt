...
function handle_search(guess, level) {
  ...
  if (level >= 1 && level <= words.length) {
    if (dict.has(guess)) {
      return {"guess": guess, "level": level, "distance":
                    iterative_levenshtein(guess, words[level - 1])};
    } else {
...
$(document).ready(function() {
  ...
  function refreshGuesses() {
    ...
    for (var guess of guessLog[level - 1]) {
      ...
      if (guess[1] === 0) {
        buildHtml = "<h4>You got it! That&rsquo;s the word!</h4>" + buildHtml;
      }
    }
  ...
  function submitGuess() {
    data = handle_search($("#guess").val(), parseInt($("#word-select")[0].value));
    if ('error' in data) {
